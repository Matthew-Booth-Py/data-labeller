...
export function PdfTextAnnotator({
  ...
  const getEntityColor = useCallback((fieldName: string): string => {
  ...
  const handleMouseUp = useCallback((e: MouseEvent) => {
    ...
    const startPage = (range.startContainer.parentElement as HTMLElement)?.closest('.react-pdf__Page');
    const endPage = (range.endContainer.parentElement as HTMLElement)?.closest('.react-pdf__Page');
    ...
    const textLayer = (range.startContainer.parentElement as HTMLElement)?.closest('.react-pdf__Page__textContent');
    if (!textLayer) {
      setPopupPosition(null);
      setPendingSelection(null);
      return;
    ...
    const pageNum = parseInt(startPage.getAttribute('data-page-number') || '1');
    ...
    if (activeEntityTypeId && activeEntityType) {
      const bbox: BoundingBoxData = {
        ...
        x: rects[0].leftPct,
        y: rects[0].topPct,
        width: rects.reduce((max, r) => Math.max(max, r.leftPct + r.widthPct), 0) - rects[0].leftPct,
        height: rects.reduce((max, r) => Math.max(max, r.topPct + r.heightPct), 0) - rects[0].topPct,
  ...
  const applyFromPopup = useCallback((entityTypeId: string) => {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        const pdfAnnotations = annotations.filter(a => a.annotation_type === 'bbox');
  ...
  }, [entityTypes, activeEntityTypeId, annotations, onActiveEntityChange, onAnnotationDelete, handleMouseUp, handleMouseDown]);

  // Render annotation highlights on a page
  const renderAnnotationHighlights = (pageNum: number) => {
    const pageAnnotations = annotations.filter(
      a => a.annotation_type === 'bbox' && (a.annotation_data as BoundingBoxData).page === pageNum
    );
  ...
  const renderSuggestionOverlays = (pageNum: number) => {
    const pageSuggestions = suggestions.filter(
      s => s.annotation_type === 'bbox' && (s.annotation_data as BoundingBoxData).page === pageNum
    );
    ...
    <div className="flex flex-col h-full" style={{ background: '#0d1117' }}>
      ...
      <div className="flex items-center justify-between px-5 py-2 border-b" style={{ borderColor: '#30363d', background: '#161b22' }}>
        <div className="flex items-center gap-2 text-sm" style={{ color: '#8b949e' }}>
          {activeEntityType ? (
            <>
              ...
              />
              <span>
                Labelling as <strong style={{ color: '#c9d1d9' }}>{activeEntityType.name}</strong> — highlight text to annotate
              </span>
            </>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-mono" style={{ color: '#8b949e', minWidth: '50px', textAlign: 'center' }}>
      ...
      >
        <Document
          ...
          loading={
            <div className="flex flex-col items-center justify-center h-96 gap-3" style={{ color: '#8b949e' }}>
              <Loader2 className="h-8 w-8 animate-spin" style={{ color: '#58a6ff' }} />
          ...
          error={
            <div className="flex flex-col items-center justify-center h-96 gap-3">
              ...
              <span style={{ color: '#484f58', fontSize: '12px' }}>{error}</span>
            </div>
          }
        >
