...
class ChunkContextualizer:
    ...
    def _get_document_excerpt(self, document: str) -> str:
        ...
        if len(document) <= self.max_context_chars:
            ...
            self._cached_doc_excerpt = (
                document[: self.max_context_chars] + "\n\n[Document truncated...]"
            )
    ...
    async def acontextualize_chunks(
        ...
        tasks = [
            self.acontextualize_chunk(document, chunk, semaphore, progress_callback, total)
            for chunk in chunks
        ]
    ...
    def contextualize_chunks_async(
        ...
        if loop and loop.is_running():
            try:
                ...
                nest_asyncio.apply()
                return asyncio.run(self.acontextualize_chunks(document, chunks, progress_callback))
            except ImportError:
                import concurrent.futures

                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run, self.acontextualize_chunks(document, chunks, progress_callback)
                    )
