...
class EvaluationService:
    ...
    async def evaluate_document(
        ...
        if classification:
            doc_type = await sync_to_async(repository.get_document_type)(
    ...
    def _calculate_row_match_score(
        ...
        if gt_hierarchy or pred_hierarchy:
            ...
            # Pred is single-element (leaf-only), GT is full path - check leaf match
                ...
                pred_hierarchy_was_string and len(pred_path_lower) == 1 and len(gt_path_lower) >= 1
            ):
        ...
        for gt in gt_group:
            ...
            if (
                self._is_hierarchy_field(field_key, value)
                or field_key == "hierarchy_path"
                or "_header" in field_key
            ):
                continue
        ...
        if isinstance(value, list):
            for idx, item in enumerate(value):
                ...
                if isinstance(item, dict):
                    ...
                    schema[prefix]["predicted"].append({"value": item, "instance": item_instance})
    ...
    def _build_field_comparisons(
        ...
        for field_name, data in comparison_schema.items():
            ...
            for match in matches:
                ...
                final_match_type = (
                    MatchType.SEMANTIC if is_match and not normalized_match else normalized_type
                )
    ...
    def _build_instance_comparisons(
        ...
        parent_groups: dict[str, dict[int, list[FieldComparison]]] = defaultdict(
    ...
    def _calculate_metrics(
        ...
        return EvaluationMetrics(
            flattened=flattened, instance_metrics=instance_metrics, field_metrics=field_metrics
        )
    ...
    def _calculate_flattened_metrics(self, comparisons: list[FieldComparison]) -> FlattenedMetrics:
        ...
        incorrect = sum(
            1 for c in comparisons if not c.is_correct and not c.is_missing and not c.is_extra
        )
        ...
        for parent, instances in instance_comparisons.items():
            ...
            gt_instances = len(
                [
                    i
                    for i in instances
                    if any(c.ground_truth_value is not None for c in i.field_comparisons)
                ]
            )
            pred_instances = len(
                [
                    i
                    for i in instances
                    if any(self._counts_as_predicted(c) for c in i.field_comparisons)
                ]
            )
            ...
            matched_comps = [
                c for inst in instances if inst.is_matched for c in inst.field_comparisons
            ]
            avg_field_accuracy = (
    ...
    def _calculate_field_metrics(
        ...
        for field_name, comps in field_groups.items():
            ...
            incorrect = sum(
                1 for c in comps if not c.is_correct and not c.is_missing and not c.is_extra
            )
