    ...
    def post(self, request, subpath: str):
        ...
        if parts == ["field-assistant"]:
            ...
            system_prompt = (
                ...
                "   The extraction_prompt MUST describe the TABLE LAYOUT STRUCTURE so the "
                "extraction model knows how to map columns to fields. Use structural "
                "descriptions, NOT document-specific header names (those change across "
                "documents). Specifically:\n"
                "   a) Describe the COLUMN GROUP STRUCTURE using positional language: "
                "'left column group', 'right column group', 'first set of columns', "
                "'second set of columns', 'columns 1-3', 'columns 4-6', etc.\n"
                "   b) **CRITICAL for split-section tables** (e.g. GAAP | Non-GAAP, "
                "Budget | Actual, Plan | Variance): If two measurement groups appear as "
                "separate column blocks on the SAME physical row, the extraction_prompt MUST "
                "say: 'Each physical table row is ONE object. Both the left column group and "
                "the right column group values for the same row belong in the SAME object. "
                "Do NOT split a single row into multiple objects — one row = one item.'\n"
                "   c) If the table has a fixed, known number of data rows visible in the "
                "image, state the count: 'This table has N data rows.'\n"
                "   d) If other similar-looking tables likely exist elsewhere in the document "
                "(e.g. reconciliation tables, supplemental schedules), add: "
                "'Extract ONLY from this specific table. Ignore any other tables with "
                "similar columns elsewhere in the document.'\n"
                ...
                "   - Field DESCRIPTIONS should explain which column group/position the field "
                "maps to (e.g. 'Value from the left column group for the first period') so "
                "the extraction model understands the mapping without document-specific names\n"
