...
class TestNormalizeHierarchyToArray:
    ...
    def service(self):
        with (
            patch("uu_backend.services.evaluation_service.get_extraction_service"),
...
class TestCompareHierarchyValues:
    ...
    def test_compare_array_to_array_exact(self, service):
        ...
        assert (
            service._compare_hierarchy_values(
    ...
    def test_compare_single_string_to_single_array(self, service):
        ...
        assert service._compare_hierarchy_values("GAAP additions", ["GAAP additions"]) is True
    ...
    def test_compare_non_matching(self, service):
        ...
        assert (
            service._compare_hierarchy_values(
                "Something else", ["GAAP additions", "Proceeds from capital"]
            )
            is False
        )
    ...
    def test_compare_case_insensitive(self, service):
        ...
        assert (
            service._compare_hierarchy_values(
                "GAAP ADDITIONS,PROCEEDS FROM CAPITAL", ["gaap additions", "proceeds from capital"]
            )
            is True
        )
...
class TestHierarchyPathMatching:
    ...
    def test_hierarchy_matches_string_exact_single_element(self, service):
        ...
        assert (
            service._hierarchy_matches_string(["GAAP R&D"], "gaap r&d") is True
        )  # Case insensitive
    ...
    def test_hierarchy_matches_string_leaf_match(self, service):
        ...
        assert (
            service._hierarchy_matches_string(
                ["GAAP additions", "Proceeds from capital-related"], "Proceeds from capital-related"
            )
            is True
        )
        ...
        assert (
            service._hierarchy_matches_string(
                ["GAAP additions", "Proceeds from capital-related"], "GAAP additions"
            )
            is False
        )
    ...
    def test_hierarchy_matches_string_whitespace_handling(self, service):
        ...
        assert (
            service._hierarchy_matches_string(
                ["GAAP R&D", "  Share-based compensation  "], "Share-based compensation"
            )
            is True
        )
...
class TestRowMatchScoring:
    ...
    def test_calculate_row_match_score_string_gt_to_array_pred(self, service):
        ...
        gt_group = [
            ...
            {"field_name": "forward_looking_estimates_table.period_1_value", "value": "(1.0)"},
            {
    ...
    def test_calculate_row_match_score_no_match_different_leaf(self, service):
        ...
        gt_group = [
            ...
            {"field_name": "forward_looking_estimates_table.period_1_value", "value": "(13.0)"},
        ]
...
class TestEndToEndScenario:
    ...
    def test_forward_looking_estimates_table_matching(self, service):
        ...
        assert (
            len(matches) >= 4
        ), f"Expected at least 4 hierarchy matches, got {len(matches)}: {matches}"
...
class TestAnnotationSuggestionServiceFlattening:
    ...
    def service(self):
        ...
        with (
            ...
            from uu_backend.services.annotation_suggestion_service import (
    ...
    def test_flatten_value_keeps_hierarchy_path_atomic(self, service):
        ...
        fields = [
            ExtractedField(
                field_name="table",
                value=[
                    {"hierarchy_path": ["Level 1", "Level 2"], "amount": "$100"},
                    {"hierarchy_path": ["Level 1", "Level 2", "Level 3"], "amount": "$200"},
                ],
                confidence=0.95,
                source_text=None,
            )
        ...
        hp_entries = [
            (name, val, inst) for name, val, inst in flat_fields if "hierarchy_path" in name
        ]

        # Should have 2 hierarchy_path entries (one per row), NOT 5 (one per path element)
        assert (
            len(hp_entries) == 2
        ), f"Expected 2 hierarchy_path entries, got {len(hp_entries)}: {hp_entries}"

        # Each entry should have the FULL array, not individual strings
        for name, val, inst in hp_entries:
            assert isinstance(
                val, list
            ), f"hierarchy_path value should be list, got {type(val)}: {val}"
    ...
    def test_flatten_value_flattens_row_arrays(self, service):
        ...
        descriptions = [
            (name, val, inst) for name, val, inst in flat_fields if "description" in name
        ]
...
class TestSchemaConsistency:
    ...
    def test_ground_truth_array_values_match_predictions(self):
        ...
        predictions = [
            {"hierarchy_path": ["GAAP R&D", "Share-based compensation"], "period_1_value": "(2.4)"}
        ]
        ...
        gt_labels = [
            ...
            {"field_name": "table.period_1_value", "value": "(2.4)", "instance_num": 1},
        ]

        extraction = MagicMock()
        extraction.fields = [
            ExtractedField(field_name="table", value=predictions, confidence=0.95, source_text=None)
        ]
    ...
    def test_same_instance_missing_and_extra_become_single_incorrect_pair(self, service):
        ...
        comparison_schema = {
            "table.hierarchy_path": {
                ...
                "predicted": [
                    {"value": ["GAAP additions", "Partner contributions, net"], "instance": 4}
                ],
            }
        }
        eval_results = {
    ...
    def test_hierarchy_leaf_match_recovers_from_missing_plus_extra(self, service):
        ...
        comparison_schema = {
            "table.hierarchy_path": {
                ...
                "predicted": [
                    {"value": ["GAAP R&D and MG&A", "Non-GAAP R&D and MG&A"], "instance": 8}
                ],
            }
        }
        eval_results = {
