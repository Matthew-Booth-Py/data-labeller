...
| Phase 2: DRF Migration + Celery | In Progress | Wave A and Wave B (`documents`, `graph`, `providers`) implemented in DRF; Celery wiring added |
...
| ingest | FastAPI | FastAPI |
| suggestions | FastAPI | FastAPI |
| tutorial | FastAPI | FastAPI |
| taxonomy | FastAPI | FastAPI |
| annotations | FastAPI | FastAPI |
| deployments | FastAPI | FastAPI |
| evaluation | FastAPI | FastAPI |

## Feature Flags

- `DJANGO_MIGRATED_GROUPS`: comma-separated migrated groups, e.g. `health,timeline,search,documents,graph,providers`
